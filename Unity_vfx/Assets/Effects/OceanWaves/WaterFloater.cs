...
public class WaterFloater : MonoBehaviour
{
    ...
    public void Init(RenderTexture heightMap, float oceanPatchSize)
    {
    ...
    void Update()
    {
        ...
        if( m_timer > SampleFrequency )
        {
            ...
            // Front
            Vector2 TextureSpacePointCoordFront = GetTextureSpaceCoordinate(this.transform.position + transform.forward);
            Vector2 TextureSpacePointCoordBack = GetTextureSpaceCoordinate(this.transform.position - transform.forward);
            
            m_positionsBuffer.SetData(new Vector2[]{TextureSpacePointCoordFront, TextureSpacePointCoordBack });
            //m_waterPointHeightSamplerCS.SetVector("PointPos", TextureSpacePointCoord);
            ...
            m_sampledForward = (SampledFront - SampledBack).normalized;

            this.transform.rotation = Quaternion.LookRotation(m_sampledForward, Vector3.up);
        }
